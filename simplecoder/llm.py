...
single unified interface. Set SIMPLECODER_PROVIDER=litellm.
"""
...
class LLM:
    ...
    def chat(
        ...
        """Send messages, stream back response, handle tools calls."""
        params: dict = {
            "model": self.model,
            "messages": messages,
            "stream": True,
        ...
        for chunk in stream:
            ...
            # accumulate tools calls across chunks
            if delta.tool_calls:
...
class LiteLLM(LLM):
    ...
    Set SIMPLECODER_PROVIDER=litellm and use LiteLLM model strings
    ...
    def chat(
        ...
        """Send messages via litellm, stream back response, handle tools calls."""
        params: dict = {
            "model": self.model,
            "messages": messages,
            "stream": True,
