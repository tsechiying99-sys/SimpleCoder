"""命令行入口：提供交互式 REPL 和一次性 prompt 模式。"""
...
def _parse_args():
    ...
    p.add_argument("-m", "--model", help="Model name (default: $SIMPLECODER_MODEL, $MODEL_NAME, or gpt-5.5)")
...
def main():
    ...
    if not config.api_key:
        ...
        console.print(
            "Set one of: OPENAI_API_KEY, DEEPSEEK_API_KEY, or SIMPLECODER_API_KEY\n"
            "\nExamples:\n"
            "  # OpenAI\n"
            ...
            "\n"
            "  # Ollama (local)\n"
            "  export OPENAI_API_KEY=ollama OPENAI_BASE_URL=http://localhost:11434/v1 MODEL_NAME=qwen2.5-coder\n"
        )
        sys.exit(1)
...
def _repl(agent: Agent, config: Config):
    ...
    console.print(Panel(
        f"[bold]SimpleCoder[/bold] v{__version__}\n"
    ...
    hist_path = os.path.expanduser("~/.simplecoder_history")
    ...
    while True:
        ...
        if user_input == "/save":
            ...
            console.print(f"Resume with: simplecoder -r {sid}")
            continue
        if user_input == "/diff":
        ...
        try:
            ...
            if streamed:
                ...
                # response wasn't streamed (came after tools calls)
...
def _show_help():
    console.print(Panel(
        ...
        "  quit           Exit SimpleCoder\n"
        "\n"
        "[bold]Input:[/bold]\n"
        "  Enter          Submit message\n"
        "  Esc+Enter      Insert newline (for pasting code)",
        title="SimpleCoder Help",
        border_style="dim",
    ))
