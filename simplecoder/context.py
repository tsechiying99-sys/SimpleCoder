...
Claude Code uses a 4-layer strategy:
  1. HISTORY_SNIP   - trim old tools outputs to a one-line summary
...
SimpleCoder implements the same idea in 3 layers:
  Layer 1 (tool_snip)   - replace verbose tools results with truncated versions
...
class ContextManager:
    def __init__(self, max_tokens: int = 128_000):
        ...
        self._snip_at = int(max_tokens * 0.50)    # 50% -> snip tools outputs
    ...
    def maybe_compress(self, messages: list[dict], llm: LLM | None = None) -> bool:
        ...
        # Layer 1: snip verbose tools outputs
    ...
    def _snip_tool_outputs(messages: list[dict]) -> bool:
        """Layer 1: Truncate tools results over 1500 chars to their first/last lines.

        This mirrors Claude Code's HISTORY_SNIP which replaces old tools outputs
    ...
    def _safe_split(messages: list[dict], keep_recent: int) -> int:
        ...
        Walk the boundary back so a 'tools' result is never separated from the
        assistant message whose tool_calls produced it - an orphaned tools
